...
<div class="d-flex justify-content-between align-items-center mb-4">
  <h1 class="h3 fw-bold" style="color: #6B3E26;">Previous Jobs</h1>
...
<div class="card shadow-sm border-0 mb-4">
  <div class="card-body bg-light rounded">
      ...
      <div class="col-md-12 text-end">
        ...
        <!-- 🔄 Reset Button -->
        <a href="{{ route('previous.bookings') }}" class="btn btn-outline-secondary ms-2">
...
<div class="card shadow-sm border-0" style="background-color: #FFF5CC; border-left: 5px solid #D39F61;">
  <div class="card-body">
    <h5 class="card-title mb-3 fw-semibold" style="color: #A86B32;">Previous Bookings<span class="ms-2">({{ count($previousBookings) }})</span></h5>
...
">
      <table class="table table-hover table-borderless align-middle">
        ...
        <tbody>
          @forelse ($previousBookings as $booking)
          @php
    // Normalize status
    $currentStatus = strtolower($booking['status'] ?? 'pending');

    // Status → Bootstrap color map
    $statusBadges = [
        'pending'       => 'warning',
        'accepted'      => 'info',
        'declined'      => 'danger',
    ...
    $badge = $statusBadges[$currentStatus] ?? 'warning';

    // Inline style (secondary uses default look)
    $style = $badge === 'secondary'
        ? ''
        : "background-color: var(--bs-{$badge}); 
           color: #fff; 
           border-radius: 6px; 
           padding: 6px 12px; 
           font-weight: 500;";
...
<td>
    <span class="badge rounded-pill px-3 py-2"
        ...
        {{ ucfirst($paymentType) }}
            ...
            <td>
    <span class="statusSelectStyle" style="{{ $style }}">
        {{ ucfirst(str_replace('_', ' ', $currentStatus)) }}
          ...
          @empty
          <tr>
            <td colspan="8" class="text-center text-muted py-4">No previous bookings found.</td>
          </tr>
          @endforelse
        </tbody>
      </table>
...
@if (count($previousBookings) > 0)
    ...
    //     // Optional: clean up any previous sorting icons (if needed)
    //     table.on('order.dt', function () {
    //         $('.dt-header-icon').remove();
    //         // ... rest of your icon handling logic
    //     });

    //     // Force initial descending sort on column 4 (5th column, 0-based index)
