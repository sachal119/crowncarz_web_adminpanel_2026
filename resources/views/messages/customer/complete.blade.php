...
<div class="container-fluid mt-4 px-2 px-md-4">
  <div class="card shadow-sm border-0 rounded-4 mx-auto" style="max-width: 900px;">
    ...
    {{-- Header --}}
    <div class="card-header bg-success bg-opacity-25 rounded-top-4 py-3 px-4 d-flex justify-content-between align-items-center">
      <h5 class="mb-0 fw-bold text-dark">
        <i class="bi bi-check-circle-fill me-2 text-success"></i> Job Complete SMS
      </h5>
    ...
    <div class="card-body p-4">
      <p class="text-secondary mb-4">
        This SMS is sent automatically when a job is marked as complete.
        You may customize the message below using the variable <code>{name}</code>.
      </p>

      <form action="{{ route('customer.job_complete.update') }}" method="POST">
        @csrf
        @method('POST')
        ...
        <div class="mb-3">
          ...
          <textarea name="message" id="smsMessage" rows="7"
            class="form-control rounded-3 shadow-sm">{{ old('message', $smsTemplate->message ?? 
"Dear {name},

Thank you for giving us the opportunity to serve you this time, looking forward to having you on board soon.
...
Email: info@crowncarz.com") }}</textarea>
        </div>
